...
export const init =
  (value:Value):[Model, Effects<Action>] => {
    const [input, fx] = TextInput.init((value == null
        ? ''
...
const updateTextInput = cursor({ get: model => model.input,
...
const styleSheet = StyleSheet.create({ json: {},
     string: {},
     number:
      { border: 'none',
...
const viewString = (value:string, address, contextStyle) =>
  html.code({ style: Style(styleSheet.string, contextStyle),
...
const viewBoolean = (value:boolean, address, contextStyle) =>
  html.code({ style: Style(styleSheet.boolean, contextStyle),
     onClick:
      event =>
        (event.altKey
        ? address(Edit)
...
const viewJSON = (value, address, contextStyle) =>
  html.code({ style: Style(styleSheet.json, contextStyle),
     onClick: forward(address, always(Edit))
    },
   [ JSON.stringify(value)
    ]
...
const viewInput = TextInput.view('input',
