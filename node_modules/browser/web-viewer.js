...
const {remove} = require('./deck/actions');
...
const IFrame = Element('iframe', {
  ...
  readyState: Field((node, current, past) => {
    ...
    if (current != past && current == 'loaded') {
      if (node.getScreenshot) {
        setTimeout(() => {
          var request = node.getScreenshot(100, 62.5, 'image/jpeg');
          request.onsuccess = IFrame.onScreenshot(node, request);
        }, 100);
      }
    }
  ...
  onThumbnailChange: Event('mozbrowserthumbnailchange'),
});
...
IFrame.onScreenshot = node => request => {
  node.dispatchEvent(new CustomEvent('mozbrowserthumbnailchange', {
    'detail': request.target.result
  }));
};
...
const WebViewer = Component(({item, items}) => {
  ...
  onThumbnailChange: WebViewer.onThumbnailChange(item),
...
WebViewer.onThumbnailChange = state => event =>
  state.merge({thumbnail: event.detail});
